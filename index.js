...
export default class Navgo {
	...
	async #open_cache() {
		const cs = globalThis?.caches
		return cs && cs.open ? cs.open(this.#cache_name) : null
	}
	async #cache_match(req) {
		const c = await this.#open_cache()
		if (c) return (await c.match(req)) || null
		return this.#mem_cache.get(this.#canonical_key(req)) || null
	}
	async #cache_put(req, res) {
		const c = await this.#open_cache()
		if (c) return c.put(req, res)
		this.#mem_cache.set(this.#canonical_key(req), res)
	}
	#parse(res, parse) {
	...
	async #fetch_and_maybe_revalidate(req, sidecar, hints = {}, signal) {
		...
		if (res.status === 304) {
			this.#write_meta(this.#canonical_key(req), { ...sidecar, ts: Date.now() })
			return (await this.#cache_match(req)) || res
		}
		if (res.ok) {
			const clone = res.clone()
			await this.#cache_put(req, clone)
			const etag = res.headers.get('ETag')
			const last_modified = res.headers.get('Last-Modified')
			const meta = { ts: Date.now(), etag, last_modified, tags: hints?.tags || [] }
			this.#write_meta(this.#canonical_key(req), meta)
		}
		return res
	}
	async #revalidate(req, sidecar, hints, signal) {
		try {
			await this.#fetch_and_maybe_revalidate(req, sidecar, hints, signal)
		} catch {}
	}
		...
		await Promise.all(
			Object.entries(plan).map(async ([as, spec]) => {
				...
				if (strat === 'cache-first' && entry && fresh) {
					...
					this.#revalidate(req, side, cache, controller.signal)
