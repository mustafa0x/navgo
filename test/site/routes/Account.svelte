...
export function before_route_leave(nav) {
    ...
    if (nav.will_unload || nav.type === 'link' || nav.type === 'nav' || nav.type === 'popstate') {
        // For SPA navigations, we can ask interactively. For unload, router will show native confirm.
        if (!nav.will_unload && !confirm('You have unsaved changes. Leave this page?')) nav.cancel()
        if (nav.will_unload) nav.cancel()
    }
}
</script>
